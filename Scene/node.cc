...
void Node::addChild(Node *theChild) {
	...
	if (m_gObject) {
        ...
        //es nodo hoja no se puede meter hijo
	} else {
		// node does not have gObject, so attach child
        //insertar en m_children a theChild y poner cual es su padre
	}
}
...
void Node::draw() {
	...
	/* =================== PUT YOUR CODE HERE ====================== */
    //si hay un objeto asociado al nodo dibujarlo, sino recorrer la lista de hijos y dibujarlos(despues de aplicar las transformaciones del padre y las de los propios hijos)

    //ojo hay que tener en cuenta las transformaciones
     //si eres un nodo hoja dibuja tu objeto y vete
     //    m_gObject->draw()
     //sino      
       /* for(list<Node *>::iterator it = m_children.begin(), end = m_children.end();
            it != end; ++it) {
