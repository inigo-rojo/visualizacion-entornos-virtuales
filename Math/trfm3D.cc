...
void Trfm3D::setRotAxis(const Vector3 & V, const Vector3 & P, float angle ) {
    //matriz de translacion inversa * matriz de rotacion * matriz de translacion al origen * lo que haya en T
    Trfm3D M;
    Vector3 A(V);
    A.normalize();
    Vector3 menosP=Vector3(-1*P[0],-1*P[1],-1*P[2]);
    M.setTrans(P);
    //establecemos la rotacion
    Trfm3D R=Trfm3D();
    R.m_c1[0]=cos(angle)+A[0]*A[0]*(1-cos(angle));
    R.m_c1[1]=A[1]*A[0]*(1-cos(angle))+A[2]*sin(angle);
    R.m_c1[2]=A[2]*A[0]*(1-cos(angle))-A[1]*sin(angle);
    R.m_c2[0]=A[0]*A[1]*(1-cos(angle))-A[2]*sin(angle);
    R.m_c2[1]=cos(angle)+A[1]*A[1]*(1-cos(angle));
    R.m_c2[2]=A[2]*A[1]*(1-cos(angle))+A[0]*sin(angle);
    R.m_c3[0]=A[0]*A[2]*(1-cos(angle))+A[1]*sin(angle);
    R.m_c3[1]=A[1]*A[2]*(1-cos(angle))-A[0]*sin(angle);
    R.m_c3[2]=cos(angle)+A[2]*A[2]*(1-cos(angle));
    M.add(R);
    M.addTrans(menosP);
    //la matriz de rotacion ya esta, pero que se hace con esto?
    //this->add(M);
    M.add(this);
    swap(M);
}
